...
exports.getToursWithin = catchAsync(async (req, res, next) => {
  ...
  const radius = unit === 'mi' ? distance / 3963.2 : distance / 6378.1; // must divide distance by radius of the earth, second half is km
  ...
  const tours = await Tour.find({
    startLocation: {
      $geoWithin: {
        $centerSphere: [[lng, lat], radius], // longitude must be first, latitude second
      },
    },
  });
  ...
  const distances = await Tour.aggregate([
    {
      $geoNear: {
        near: {
          type: 'Point',
          coordinates: [lng * 1, lat * 1], // times by 1 to convert to a number
        },
